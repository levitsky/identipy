...
def main(inputfile, preFDR, FDR, RT_type, min_charge, max_charge, valid_proteins_input):
    ...
    descriptors = []
    descriptors.append(Descriptor(name='RT difference', formula='(peptide.RT_exp - peptide.RT_predicted)', group='A', binsize='auto'))
    descriptors.append(Descriptor(name='precursor mass difference', formula='peptide.mass_diff()', group='A', binsize='auto'))
    ...
    descriptors.append(Descriptor(name='isotopes mass difference', formula='round(peptide.massdiff, 0)', group='A', binsize='1'))
    ...
    def prepare_hist(descriptors, copy_peptides):
        for descriptor in descriptors:
            ...
            if descriptor.group == 'A':
                ...
                if lbin == rbin:
                    rbin = lbin + binsize
                descriptor.hist = np.histogram(descriptor.array, bins=np.arange(lbin, rbin, binsize))
        return descriptors
...
main(inputfile, preFDR, FDR, RT_type, min_charge, max_charge, valid_proteins_input)
